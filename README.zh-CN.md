...
DAO、映射与可观察缓存接口。Alpaca Basic 现可作为用户主动选择的图表源；用户
凭据通过 Android Keystore 加密保存，分页 K 线写入 Room，并明确标注数据来自
实时 IEX 单一交易所、不是全美综合市场数据。应用现已按交易所时间将已完成的
1 分钟 K 线本地聚合为 5 分钟 K 线，并与源数据原子写入 Room；同时基于有效
...
提供股票查询和首个详情页切片，将
行情、估值、支撑/阻力、技术指标和本地缓存的多周期 K 线组合展示。Vico 图表
使用中性蜡烛样式和独立缩放的 IEX 成交量柱，时间按设备本地时区显示，并支持
平移、缩放及回到最新数据。自选列表持久化、筛选、概率线叠加和 ONNX 推理仍待
