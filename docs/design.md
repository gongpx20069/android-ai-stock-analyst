...
| 9 | **K线 OHLC**(二期) | §4.2 详情页⑥(收起) | Candlestick | 中性单色 + 实/空心区分涨跌,红绿留给估值 |
...
- **图表**:② 区间点图 + ②b 支撑压力位图(共振位加粗,数据来自 [`analysis.md` §1.2](analysis.md#12-支撑压力位必做4-种算法)) + ⑤ 评级分布 + ⑥ K线 + ⑦ RSI/52周位置仪表 + ⑤ ML 概率仪表(见 [`analysis.md` §4.3](analysis.md#43-结果可视化与产品红线))。
...
> **选型结论**:主用 fl_chart 覆盖 P0/P1 估值与盈亏图,K线二期用 candlesticks 或 syncfusion;MVP 可先不做 K线。每张图的配色/A11y 规则见 [MASTER §5](design-system/ai-stock-analyst/MASTER.md),本节只管"画哪些图"。
