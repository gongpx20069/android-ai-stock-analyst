...
### 4.3 数据层:沿用 data-sources.md 方案③
- 实时价 → 腾讯/新浪(国内快);基本面/分析师 → yfinance 后端缓存。
- AI agent 和 ML 都从这层取数,不重复造数据通路。

### 4.4 可视化层:沿用 visualization.md + ML 四图
- 估值图(上行空间条形/区间点图)为主 + AI 决策卡 + ML 概率仪表/回测图。
...
| 来源项目 | 借鉴什么 | 怎么用 |
|---------|---------|--------|
| TradingAgents(90k) | 多智能体分工 + 辩论架构 | 砍成 3+1 精简版 |
| TradingAgents-AShare | 产品形态:自选股/定时/持仓/BYOK多厂商/决策卡 | 直接对标,做 Android 美股版 |
| FinMem(916) | 分层记忆 | 二期可给 agent 加"历史判断记忆" |
| robertmartin8(1.9k) | scikit-learn 克制的选股预测 | ML 层照此轻量化 |
| stefan-jansen(19k) | ML 特征工程方法 | LightGBM 特征设计参考 |
| huseinzol05(9.4k) | 知道 LSTM/RL 长啥样 | **当反面教材**:知道但不用 |

---

## 7. ❓待你拍板

- [ ] Q1. AI 层走**精简多智能体(3+1)**,认可吗?还是想要更简单的"单次AI解读"(1个agent)?
- [ ] Q2. ML 层定位为"**辅助信号、不单独下结论**",认可吗?
- [ ] Q3. 要不要像 AShare 那样做**定时分析+持仓追踪**(你已有每日WeChat监控,可整合进App)?
- [ ] Q4. 多智能体烧 token,你的 Azure 额度够频繁用吗?要不要加"省钱模式=只跑1个agent"?

---

## 8. 一句话总结

**别造重型量化框架。最合适的是:抄 TradingAgents 的多智能体思想但砍到 3+1 个 agent(吃你的 Azure key、内嵌你的估值纪律、输出可解释决策卡),ML 只做 LightGBM 涨跌概率当辅助信号,数据走方案③,可视化以估值图为主。本质是做一个"TradingAgents-AShare 的 Android 美股轻量版"。**
